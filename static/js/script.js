  ...
  const DUMMY_HISTORY = [
    {
      ...
      clauses: [
        {
          id: 'clause-1',
          title: '🔴 Unlimited Liability (Sec. 5)',
          severity: 'high',
        ...
        {
          id: 'clause-2',
          title: '🟠 Automatic IP Transfer (Sec. 7)',
          severity: 'medium',
    ...
    },
    {
      ...
      clauses: [
        ...
        {
          id: 'clause-2',
          title: '🟠 Strict Non-Solicit Covenant',
          severity: 'medium',
    ...
    },
    {
      ...
      clauses: [
        {
          id: 'clause-1',
          title: '🔴 Uncapped Patent Indemnity',
          severity: 'high',
  ...
  function loadContract(contractId) {
        ...
        div.innerHTML = `
          ...
          <div class="mock-tooltip">
            <div class="mock-tooltip__header">
              <span class="mock-tooltip__risk-badge ${clause.severity === 'medium' ? 'mock-tooltip__risk-badge--warning' : ''}">⚠️ ${badgeText}</span>
    ...
    if (clauseListContainer) {
      ...
      contract.clauses.forEach(clause => {
        ...
        const dotColor = clause.severity === 'high' ? '🔴' : clause.severity === 'medium' ? '🟠' : '🟢';
    ...
    if (submitAnalysisBtn) {
      submitAnalysisBtn.addEventListener('click', () => {
        ...
        setTimeout(() => {
          ...
          const fileName = uploadedFileName.textContent || 'Uploaded_Contract.pdf';
          ...
          const newScanItem = {
            ...
            type: fileType,
            category: 'Today',
            riskScore: Math.floor(Math.random() * (94 - 64 + 1)) + 64,
            riskLevel: 'Medium Risk',
            summary: `This contract (${fileName}) was uploaded and scanned successfully. ContractShield identified two key vulnerabilities regarding indemnification rules and standard termination conditions that require immediate redrafting.`,
            obligations: [
              { name: 'Confidentiality Obligations', status: 'Compliant', severity: 'success' },
              { name: 'Governing Law', status: 'Standard', severity: 'success' },
              { name: 'Indemnity Limit', status: 'Missing Cap', severity: 'danger' },
              { name: 'Termination for Convenience', status: 'Unbalanced', severity: 'warning' }
            ],
            clauses: [
              {
                id: 'clause-new-1',
                ...
                severity: 'high',
                text: 'The Indemnifying Party shall hold harmless and indemnify the other Party from and against any and all claims, liabilities, and legal defense costs without any limitation or cap.',
                risk: 'Exposes you to unlimited legal and financial liability in indemnity scenarios, bypassing all standard contract liability caps.',
                explanation: 'Indemnification obligations are standard in vendor agreements, but they must be subject to an overall liability limit. Accepting uncapped indemnity creates significant operational risk.',
                recommendation: 'Request a standard cap equal to 2x the annual contract value, or carve out specific categories.'
              },
              {
                id: 'clause-new-2',
                title: '🟠 One-Sided Termination (Convenience)',
                severity: 'medium',
                text: 'The Client may terminate this Agreement at any time with or without cause by providing ten (10) days written notice. Provider shall have no reciprocal right to terminate.',
                risk: 'Allows the other party to cancel the deal with minimal notice while locking you into long-term delivery obligations.',
                explanation: 'Balanced agreements generally feature reciprocal termination clauses or longer notice periods (e.g., 30 to 60 days) to prevent sudden, unmitigated business interruptions.',
                recommendation: 'Make the notice period 30 days and negotiate mutual termination for convenience rights.'
              }
            ]
          };

          if (newScanItem.riskScore > 80) {
            newScanItem.riskLevel = 'High Risk';
          }
